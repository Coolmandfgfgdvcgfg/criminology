...
textService = getgenv().textService
inputService = getgenv().inputService
                ...
                if option.type ~= "button" and option.flag and not option.skipflag then
                    if option.type == "toggle" then
                        task.spawn(function()
                            ...
                            for i = 1, option.subcount do
                                local subOption = option[i]
                                if subOption and subOption.type == "bind" then
                                    subOption:SetKey(Config[subOption.flag])
                                end
                            end
                        end)
        ...
        if option.type ~= "button" and option.flag and not option.skipflag then
            if option.type == "toggle" then
                ...
                for i = 1, option.subcount do
                    local subOption = option[i]
                    if subOption and subOption.type == "bind" then
                        Config[subOption.flag] = subOption.key
                    end
                end
            elseif option.type == "color" then
		...
		if binding then
			...
			if (input.KeyCode.Name == option.key or input.UserInputType.Name == option.key) and not binding then
				...
				if option.mode == "toggle" then
					...
					pcall(function()
						--print(library.flags[option.flag])
						--option.callback(library.flags[option.flag], 0)
						if option.OgOption then
							option.OgOption:SetState(library.flags[option.flag])
						else
							option.callback(library.flags[option.flag], 0)
						end
					end)
		...
		function column:AddSection(title)
			...
			function section:AddToggle(option)
				...
				function option:AddBind(subOption)
					subOption = typeof(subOption) == "table" and subOption or {}
					subOption.sub = true
					subOption.callback = option.callback
					subOption.flag = option.flag .. " Bind" -- Ensure unique flag for suboption
					subOption.OgOption = option
					subOption.subpos = self.subcount * 24
					function subOption:getMain() return option.main end
					self.subcount = self.subcount + 1
